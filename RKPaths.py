...
class RKPath:
    ...
    def PlanPath(self, q_start, q_goal):
        ...
        # [path_LSL, L_LSL]  = self.LSL(q_s,q_g,self.k_max,self.sig_max,self.mu)
        # [path_RSL, L_RSL]  = self.RSL(q_s,q_g,self.k_max,self.sig_max,self.mu)
        # [path_LSR, L_LSR]  = self.LSR(q_s,q_g,self.k_max,self.sig_max,self.mu)
        [path_RLR, L_RLR]  = self.RLR(q_s,q_g)
        # [path_LRL, L_LRL]  = self.LRL(q_s,q_g,self.k_max,self.sig_max,self.mu)
    ...
    def get_spiral_center(self, q, dir, k, sig):
        ...
        cent = util.Tf(q[:2],q[3]).dot(np.array([O_i[0], O_i[1], 1]))
    ...
    def arc_right(self, center, q_start, q_end, k_max, sig_max, mu):
        ...
        if delta == 0:
            print("Straight path")
            path = np.zeros((2, 20))
            d = np.linspace(0, 2 * r * np.sin(mu), 20)
            path[0, :] = q_start[0] + d * np.cos(q_start[2])
            path[1, :] = q_start[1] + d * np.sin(q_start[2])
            L = 2 * r * np.sin(mu)
        elif ((0 < delta < delta_min) or k_max > np.sqrt(2 * sig_max * np.pi)) and (q_start[3] == 0 and q_end[3] == 0):
            print("loop insurance")
            ...
            rev_spiral = util.euler_spiral_point(0, -np.sqrt(delta / new_sig))
